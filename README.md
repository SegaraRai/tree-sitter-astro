...
This repo is already wired for fuzzing in CI via [`.github/workflows/fuzz.yml`](.github/workflows/fuzz.yml) (it uses Tree-sitter's built-in `tree-sitter fuzz` harness).
...
- LLVM/clang (AddressSanitizer runtime must be discoverable on `PATH`)

Run:

```powershell
./script/fuzz-sanitize.ps1 -Iterations 5000 -Edits 200
```

Notes:

- This catches many memory bugs (use-after-free, out-of-bounds, etc.).
- Leak detection via sanitizers is limited on Windows; if you specifically want leak reports, use WSL/Linux.

### WSL / Linux (ASan/UBSan + leak detection)

```bash
./script/fuzz-sanitize.sh
```

If a sanitizer report triggers, `tree-sitter fuzz` will exit non-zero and print a stack trace. Re-run with `--log-graphs` or narrower `--include` to minimize/reproduce.
...
Astro's special HTML format is _not_ JSX/TSX, but rather an extension of HTML. This grammar is an extension of the grammar from [tree-sitter-html](https://github.com/tree-sitter/tree-sitter-html).
